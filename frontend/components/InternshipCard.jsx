...
export default function InternshipCard({ internship, index }) {
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-green-500/40 hover:bg-white/[0.07] transition-all duration-300 backdrop-blur-sm"
    >
      {/* Location badge */}
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {internship.skills.map((skill) => (
          <span key={skill} className="text-xs bg-green-500/10 text-green-400 border border-green-500/20 px-2 py-1 rounded-full">
            {skill}
          </span>
        ))}
      </div>
      ...
      <Link
        to={`/apply?domain=${encodeURIComponent(internship.title)}`}
        className="block w-full text-center bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white text-sm font-medium py-2.5 rounded-xl transition-all duration-300 group-hover:shadow-lg group-hover:shadow-green-500/25"
      >
        Apply Now
      </Link>
    </motion.div>
