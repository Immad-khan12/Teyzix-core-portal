  ...
  if (submitted) return (
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        ...
        <p className="text-gray-400">Thank you {form.name}! We'll review your application and get back to you at {form.email}.</p>
      </motion.div>
    </div>
  )
    ...
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 relative">
          ...
          <div
            className="absolute top-4 left-0 h-px bg-green-500 z-0 transition-all duration-500"
          ...
          {steps.map((s, i) => (
            <div key={s} className="relative z-10 flex flex-col items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all ${
                ...
                : 'bg-[#0a0a0f] border-white/20 text-gray-500'
              }`}>{i < step ? '✓' : i + 1}</div>
        ...
        >
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-2">Preferred Domain</label>
                <select
                  name="domain"
                  value={form.domain}
                  onChange={update}
                  className="w-full bg-[#0a0a0f] border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-green-500/50 transition"
                >
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-2">Cover Letter / Message</label>
                <textarea
                  name="message"
                  value={form.message}
                  onChange={update}
                  rows={5}
                  placeholder="Tell us about yourself, your skills, and why you want to join TEYZIX CORE..."
                ...
                />
                <p className="text-right text-xs text-gray-500 mt-1">{form.message.length} / 500</p>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between mt-8">
            {step > 0 ? (
              <button onClick={back} className="px-6 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-white transition">Back</button>
            ) : <div />}
            {step < steps.length - 1 ? (
              <button onClick={next} className="px-6 py-2.5 rounded-xl bg-green-500 hover:bg-green-400 text-white font-medium transition">Continue</button>
            ) : (
              <button
                onClick={handleSubmit}
                disabled={loading}
                className="px-8 py-2.5 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white font-medium transition disabled:opacity-50"
              >
