    ...
    <div className="min-h-screen pt-20 pb-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={handleLogout} className="flex items-center gap-2 bg-red-500/20 text-red-400 px-4 py-2 rounded-xl hover:bg-red-500/30 transition border border-red-500/20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: 'This Week', value: applications.filter(a => {
              const d = new Date(a.createdAt)
              return (new Date() - d) < 7 * 24 * 60 * 60 * 1000
            }).length },
            { label: 'Domains Applied', value: new Set(applications.map(a => a.domain)).size },
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-green-500/50 transition text-sm"
            />
          </div>
          <select
            value={domainFilter}
            onChange={e => setDomainFilter(e.target.value)}
            className="bg-[#0a0a0f] border border-white/10 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-green-500/50 transition text-sm"
          >
